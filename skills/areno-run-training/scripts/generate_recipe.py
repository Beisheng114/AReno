...
# Built-in algorithm names (synced from areno.api.algorithms._register_builtin).
...
def _validate_with_areno(algo: str, config_kwargs: dict[str, Any], warnings: list[str]) -> None:
    ...
    try:
        ...
        cls = cls_map[algo]
        valid = {k: v for k, v in config_kwargs.items() if k in {f.name for f in __import__("dataclasses").fields(cls)}}
        cls(**valid)
...
def generate_recipe(
    *, algo: str, gpus: int = 0, tp_size: int = 0, context_len: int = 0,
    ...
    if auto:
        ...
        # These are lower than areno's CLI defaults (which assume 8×80GB GPUs
